...
export const razorpayCreateOrder = async (req, res) => {
    try {
        ...
        const razorpayOrder = await RazorpayService.createOrder(Number(amount), currency, String(orderId));
        res.json({
            success: true,
            data: {
...
export const paypalCreateOrder = async (req, res) => {
    try {
        ...
        const paypalOrder = await PayPalService.createOrder(amount, currency, String(order.id), returnUrl, cancelUrl, `pp-create-${order.id}-${Date.now()}`);
...
export const paypalCaptureOrder = async (req, res) => {
    try {
        ...
        const captureResult = await PayPalService.captureOrder(String(paypalOrderId), `pp-capture-${order.id}-${paypalOrderId}`);
        ...
        const matchedUnit = purchaseUnits.find((unit) => String(unit?.reference_id || '') === String(order.id));
        if (!matchedUnit) {
