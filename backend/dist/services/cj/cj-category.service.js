...
export class CjCategoryService {
    ...
    static async syncCategories() {
        ...
        const firstList = response.data?.data || [];
        const rows = [];
        for (const first of firstList) {
            ...
            const secondList = first.categorySecondList || first.children || [];
            for (const second of secondList) {
                ...
                const thirdList = second.categoryThirdList || second.children || [];
...
//# sourceMappingURL=cj-category.service.js.map