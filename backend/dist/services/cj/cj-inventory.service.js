...
export class CjInventoryService {
     ...
     */
    static async syncProductInventory(productId) {
        const cjProduct = await prisma.cjProduct.findUnique({ where: { product_id: productId } });
        if (!cjProduct)
            throw new Error(`No CJ product record for product ID ${productId}`);
        const detail = await CjProductService.getProductDetail(cjProduct.cj_pid);
        if (!detail)
            throw new Error(`CJ product ${cjProduct.cj_pid} not found`);
    ...
    static async syncAllInventory() {
        ...
        const results = await Promise.allSettled(cjProducts.map((cp) => this.syncProductInventory(cp.product_id)));
        const succeeded = results.filter((r) => r.status === 'fulfilled').length;
        const failed = results.length - succeeded;
        return { succeeded, failed };
    }
}
//# sourceMappingURL=cj-inventory.service.js.map