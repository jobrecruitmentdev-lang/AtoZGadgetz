...
export class CjProductService {
    ...
    static DETAIL_CACHE_TTL_MS = 1000 * 60 * 60; // 1 hour
    static detailCache = new Map();
    /**
     * Search for products in CJ Dropshipping catalog with resource filters
     */
    static async searchProducts(keyword, pageNum = 1, pageSize = 20, filters) {
        ...
        if (token === 'SANDBOX_DEMO_TOKEN') {
            ...
            if (filters?.minPrice !== undefined)
                list = list.filter(p => p.sellPrice >= (filters.minPrice || 0));
            if (filters?.maxPrice !== undefined)
                list = list.filter(p => p.sellPrice <= (filters.maxPrice || 99999));
     ...
     */
    static async huntProducts(keyword, minImages = 3, pageNum = 1, pageSize = 20, filters) {
     ...
     */
    static countImagesInDetail(detail) {
        if (!detail)
            return 0;
    ...
    static async importProduct(cjPid, categoryId, subcategoryId, markupPercentage = 2.0) {
        ...
        if (Array.isArray(productImages) && productImages.length > 0) {
            ...
            imagesList = cjProductDetails.productImageSet.split(',').map((s) => s.trim()).filter(Boolean);
        }
        if (imagesList.length === 0) {
            const single = productImage || bigImage || cjProductDetails.imageUrl || cjProductDetails.thumbnail;
            if (single)
                imagesList.push(single);
        }
        if (imagesList.length === 0) {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (imagesList && imagesList.length > 0) {
                const imagePromises = imagesList.map((imgUrl, index) => {
