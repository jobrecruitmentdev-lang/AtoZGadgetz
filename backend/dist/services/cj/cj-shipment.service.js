...
export class CjShipmentService {
    ...
    static async syncShipment(orderId) {
        const cjOrder = await prisma.cjOrder.findUnique({ where: { order_id: orderId } });
        if (!cjOrder)
            throw new Error(`No CJ order found for order ID ${orderId}`);
    ...
    static async syncAllActiveShipments() {
        ...
        const results = await Promise.allSettled(cjOrders.map((co) => this.syncShipment(co.order_id)));
        return results;
    }
    static async handleWebhook(payload) {
        const { orderNumber, orderStatus, trackingNumber, carrierName, trackingUrl } = payload;
...
//# sourceMappingURL=cj-shipment.service.js.map