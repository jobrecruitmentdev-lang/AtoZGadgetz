...
export class CjOrderService {
    ...
    static async placeOrder(orderId) {
        ...
        const products = order.items
            .filter((item) => item.product.fulfillment_type === 'cj' && item.product.cj_product?.cj_vid)
            .map((item) => ({
        ...
        const countryCode = order.address.country?.toLowerCase().includes('india') ? 'IN' : 'US';
        const payload = {
        ...
        const response = await cjHttp.post(`${this.API_BASE_URL}/shopping/order/createOrder`, payload, { headers });
    ...
    static async submitOrder(cjOrderId, headers) {
        const response = await cjHttp.post(`${this.API_BASE_URL}/shopping/order/submitOrder`, { orderId: cjOrderId }, { headers });
        return response.data;
    }
    static async cancelOrder(cjOrderId) {
        const headers = await CjAuthService.getAuthHeaders();
        const response = await cjHttp.post(`${this.API_BASE_URL}/shopping/order/cancelOrder`, { orderId: cjOrderId }, { headers });
        return response.data.code === 200;
    }
    static async getOrderDetail(cjOrderId) {
        const headers = await CjAuthService.getAuthHeaders();
...
//# sourceMappingURL=cj-order.service.js.map